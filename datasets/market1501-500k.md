...
| | |
|---|---|
| On disk | ✅ downloaded and linked into the Market root (§1) |
...
| Measured | ✅ **87 runs** (12 encoders x 7 heads, + 4 of 12 euclidean), 2026-09-08. Best mAP **60.32** / R1 **81.74** — radio-so400m-256x128 + arcface; best frozen **7.30** (siglip2-giantopt-256). The memory and encoding notes in §7 held: it cost ~4 h of encoding and the score matrix is the reason the protocol stays 🚧 |
| Gallery-size decay | The point of the page, now measured. Against the same encoder's plain Market number, a 27x gallery retains **0.53-0.84** of mAP, and the retention rises monotonically with how supervised the head is: 0.53-0.70 at `head = none` or `pca`, 0.59-0.84 with ArcFace. A stronger head does not merely score higher, it decays more slowly |
