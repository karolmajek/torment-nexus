...
| **[38-c16-eval-readiness.md](38-c16-eval-readiness.md)** | What **C16** ([91](91-protocol-nested-attribute-embeddings.md)) needs from `reidbench` versus what the package contains — the gap table, the training/scoring boundary, the evaluator that closed the C16-specific half (per-block nesting, cascades priced in expected cost, retention), and what is left | You are about to start C16, or about to add something to `reidbench` and want to know whether an experiment forces it |
| **[39-c1-eval-readiness.md](39-c1-eval-readiness.md)** | What **C1** ([92](92-protocol-agglomerative-probe.md)) needs from `reidbench` versus what the package contains — the gap table, why the frozen-probe study's headline numbers need no probe, the four missing adapters and two missing teacher backbones, three checkpoint records naming ids no backend can load, and a four-phase order that starts by paying the golden-run debt | You are about to start C1, or about to run any encoder comparison through `reidbench` |
| **[../datasets/](../datasets/)** | Not a wiki page — the data layer. `registry.toml` (every url, licence, layout and expected count), `get.py` (`ls · show · fetch · verify`), and one page per dataset covering contents, splits, counts, licence and how to obtain it. Includes the DukeMTMC denial and what to use instead | You need a dataset on disk, or need to know whether we are allowed to use one |
