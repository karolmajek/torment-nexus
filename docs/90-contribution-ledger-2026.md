...
4. **Build the feature cache first.** Half of it already exists — `reidbench`, plus cached SigLIP2-g and backbone embeddings on VeRi. Per [35-frameworks-toolboxes.md](35-frameworks-toolboxes.md) §9, build it on CLIP-ReID + pytorch-metric-learning; no toolbox provides the probe harness or the open-set protocol. That file's §7 argues C12 should be written as an installable package with a licence-provenance index from the first commit — same work, but it lands as the library the field is missing. The implementable form of that argument is now [36-eval-package-design.md](36-eval-package-design.md): `reidbench`, with its v0.2 milestone scoped precisely to what C3 + C14 need.
