...
**The "yet another ReID benchmark framework" slot is taken — twice.** arXiv 2601.20598 (Jan 2026) already ran 11 models × 9 datasets across supervised / self-supervised / language-aligned paradigms, with public code. Adding agglomerative encoders to that table is a *row*, not a paper. Since then a second occupant arrived from the MLLM side: MMReID-Bench → VP-ReID (arXiv 2508.06908) scores 15 multimodal LLMs across 10 ReID modalities against TransReID/IRRA baselines ([mmreid-bench-kb.md](../field/mmreid-bench-kb.md)). Breadth benchmarking is now crowded from two directions — and **neither occupant scores rejection, calibration, or threshold behaviour**, which is exactly the gap this ledger's recommended package attacks.
...
| Merged source | What it was | What survived into here |
|---|---|---|
| **TCSVT contribution portfolio** | 15 candidates (C1–C15) scored Value / Work / Resources for one TCSVT submission | The ID space, the scoring scale, the Pareto machinery, the packages, §6's marginal-cost lens |
| **Paper-ideas Pareto list** | 6 research directions (ideas 01–06) scored effort vs. impact, with a multi-year sequencing plan | Two candidates the C-list was missing (now C16, C17), the gap-provenance citations, the long-horizon sequencing in §9 |
...
| Existing assets | VeRi-776 downloaded; SigLIP2-g and backbone embeddings cached; `reidbench` with mAP / CMC / per-query AP; 35-file ReID wiki | this wiki and the `reidbench` package |
...
|---|---|---|
| **C1** | Agglomerative VFMs as frozen ReID encoders | C-RADIOv4, RADIOv2.5, EUPE have never been measured on ReID; they are the one encoder family missing from 2601.20598. Protocol: [92](92-protocol-agglomerative-probe.md) — **zero-shot rows 2026-08-23** ([§12](92-protocol-agglomerative-probe.md#12-first-measurements--2026-08-23)), **linear / ArcFace / PCA probes 2026-08-24** ([§13](92-protocol-agglomerative-probe.md#13-the-probes--2026-08-24)): frozen C-RADIOv4 + one affine head reaches 0.7181 mAP / 0.8872 R1 on `market1501/official@1`. **The teacher ablation (§7) ran 2026-08-26** ([§14](92-protocol-agglomerative-probe.md)) and answers H1/H2 in-domain-only, falsifying H4. The matrix now stands at **840 of 980** cells — 14 encoders x 7 heads x 9 datasets — in [`results/table.md`](../../results/table.md); what is left for C1 is EUPE-B and the open-set check ([92 §14.9](92-protocol-agglomerative-probe.md)) |
...
| **C4** | Metric-utility mismatch, measured in a real tracker | Do encoder rankings by mAP survive as rankings by HOTA / AssA / long-gap recovery inside SOMA? Prediction: they do not. **Restated 2026-08-20** — SOMA's own two-encoder data preserves *order* but not *magnitude*, and the magnitude is host-dependent; see the gate note in §9.4 and [soma-kb.md](../field/soma-kb.md) §4.4 |
...
| id | Contribution | One-line pitch |
|---|---|---|
| **C12** | Open evaluation harness release | Reproducibility asset; supports every other item. **Scoped as a product** since 2026-08-19: built as an installable MIT package with a licence-provenance index rather than an internal `eval.py`, which also makes it the library the field lacks ([35](../field/35-frameworks-toolboxes.md) §7). Publication path — a SoftwareX OSP after the main paper, realistically **70–100 pkt, not 200** ([80](80-publication-venue-2024.md) §8) |
...
- **C16.** [mrl-kb.md](../field/mrl-kb.md) §12.1 and [disentangled-attribute-embeddings-kb.md](../field/disentangled-attribute-embeddings-kb.md) §7.3 flag the *same* unpublished combination from opposite directions: no MRL-for-ReID work exists at all, and nobody has nested *within* a named attribute block. Two independently confirmed empty cells that intersect at one design.
- **C17.** [70-open-problems-2026.md](../field/70-open-problems-2026.md) §3 names it as the crux the field's 2026 paradigm study leaves unsolved — "everything else follows if you can fine-tune without erasing the prior". Seven DG mechanisms already attack it with no winner, which is exactly why its Work and Risk are high.
...
There is also a genuine technical coupling that makes the ordering C3/C14-then-C16 better than the reverse: [mrl-kb.md](../field/mrl-kb.md) §12.4 flags that HALO's radial regulariser assumes one fixed embedding dimension, and whether it composes with per-level Matryoshka renormalisation is untested. If C14 is built first, C16 inherits a working calibration head and that open question becomes a headline ablation. If C16 is built first, C14 must be retro-fitted onto a nested embedding whose per-level normalisation is itself the most common silent bug in the MRL family ([mrl-kb.md](../field/mrl-kb.md) §3.4). Doing the harder composition second is strictly better.
...
- **The calibration / rejection lane is the field's most-flagged blind spot.** [70-open-problems-2026.md](../field/70-open-problems-2026.md) ranks it #2 and "nearly untouched"; [10-taxonomy-merged.md](../field/10-taxonomy-merged.md) calls axis F's open-world value "the biggest blind spot in the taxonomy"; [30-methods-catalog.md](../field/30-methods-catalog.md) §2 notes nobody has tried swapping ReID's cross-entropy term for a distance-based one. Both ledgers put it at or near the top.
...
4. **C4 as a hard gate at the halfway mark.** If encoder rankings by mAP and by long-gap recovery turn out to *agree*, the P2 headline claim dies. Test it early with two encoders, not at the end with ten.
   ...
   > **Gate warning, 2026-08-20.** SOMA's updated README ([soma-kb.md](../field/soma-kb.md) §4.4) has effectively pre-run this two-encoder test — and **rank order agreed**: PersonViT beats OSNet-AIN on Market mAP (0.9872 vs 0.9733), on HOTA (37.4 vs 36.7) and on 5 s recovery (44% vs 35%). A naive rank-correlation experiment would therefore *confirm* the null and kill the claim as currently worded. The claim that SOMA's data actually supports is stronger and survives: **the mapping from retrieval quality to system utility is non-proportional and host-dependent.** A 1.4 pp mAP difference — noise by retrieval standards — is worth 9 pp of 5 s recovery inside SOMA-R and **0 pp inside BoostTrack++-R fed the identical features.** So C4 must be run as a grid of {encoders} x {>= 2 host trackers} and reported as effect size, not as a rank correlation on one tracker. That is a design change, not a scope change: the second host is a shim, and BoostTrack++-R already exists in SOMA's research repo.
5. **C16 after the gate.** Either as P2′'s method column under the §8 decision rule, or as the next paper — where it also inherits a working calibration head and turns [mrl-kb.md](../field/mrl-kb.md) §12.4's open question into an ablation (§5.1). Protocol already written: [91](91-protocol-nested-attribute-embeddings.md).
...
2. **Write the missing protocol.** C3 and C14 carry the paper and have no protocol document, while the two dominated candidates (C16, C1) both have one. That inversion is the biggest process risk on this page — a `95-protocol-calibrated-rejection.md` covering the OpenOOD-derived split discipline, the metric set (AUROC / FPR@95 / ECE / DIR@FAR), and the HALO head is the next thing to write. Sources are already assembled in [open-world-rejection-calibration-kb.md](../field/open-world-rejection-calibration-kb.md) §3 and §9 and [halo-loss-kb.md](../field/halo-loss-kb.md).
...
4. **Build the feature cache first.** Half of it already exists — `reidbench`, plus cached SigLIP2-g and backbone embeddings on VeRi. Per [35-frameworks-toolboxes.md](../field/35-frameworks-toolboxes.md) §9, build it on CLIP-ReID + pytorch-metric-learning; no toolbox provides the probe harness or the open-set protocol. That file's §7 argues C12 should be written as an installable package with a licence-provenance index from the first commit — same work, but it lands as the library the field is missing. That argument is now built: [`reidbench/`](../../reidbench/), whose remaining gaps against each study are tracked in [38-reidbench-owed.md](38-reidbench-owed.md).
...
| ~~Is CrowdTrack obtainable?~~ **Answered 2026-08-31 — yes, and cheaply** | Public and ungated on Hugging Face under the authors' own account (`Loseevaya/CrowdTrack`), **Apache-2.0**, 32 GB over 33 archives, no agreement and no Baidu account. Verified live, not read off a README: [datasets/crowdtrack.md](../../datasets/crowdtrack.md). **C4's access risk is closed**, and the follow-on worry about the train/test split closed with it: the official Baidu share groups the sequences into train and test, so the split exists — it is the HF mirror that flattens it. Bytes from HF, split from Baidu, both cheap. What is left is clerical: transcribe the two listings onto that page before the first C4 run, or a number over all 33 sequences will look comparable to SOMA's `CrowdTrack/train` and not be |
| ~~How far along is the attribute-embedding code, really?~~ **Answered 2026-08-21**, by reading the tree | **The model code is not in `reidbench` at all**; what was there is `truncate` and its test. The *evaluation* half of C16 is now built (per-block nesting, cascade-vs-expected-cost, retention), so the marginal discount survives on the eval side and the training side must be costed from zero. P2′ still rests on where the DiCo/MRL training code actually lives |
...
- [00-index-reid-2026.md](../00-index-reid-2026.md) · [10-taxonomy-merged.md](../field/10-taxonomy-merged.md) · [30-methods-catalog.md](../field/30-methods-catalog.md) · [35-frameworks-toolboxes.md](../field/35-frameworks-toolboxes.md) · [50-benchmarks-datasets.md](../field/50-benchmarks-datasets.md) · [60-finetuning-question.md](../field/60-finetuning-question.md) · [70-open-problems-2026.md](../field/70-open-problems-2026.md) · [80-publication-venue-2024.md](80-publication-venue-2024.md)
...
- [mrl-kb.md](../field/mrl-kb.md) §3.4, §7.3, §12.1, §12.4 · [disentangled-attribute-embeddings-kb.md](../field/disentangled-attribute-embeddings-kb.md) §7.3, §7.4 · [halo-loss-kb.md](../field/halo-loss-kb.md) · [agglomerative-vfm-kb.md](../field/agglomerative-vfm-kb.md) §7 · [foundation-model-reid-kb.md](../field/foundation-model-reid-kb.md) §6 · [reid-in-mot-kb.md](../field/reid-in-mot-kb.md) §2 · [openood-kb.md](../field/openood-kb.md) §10 · [open-world-rejection-calibration-kb.md](../field/open-world-rejection-calibration-kb.md) §3, §9 · [soma-kb.md](../field/soma-kb.md) §6, §8 · [gallery-and-evaluation-kb.md](../field/gallery-and-evaluation-kb.md)
...
- arXiv 2508.06908 — MMReID-Bench (Aug 2025) / VP-ReID (Nov 2025), 15 MLLMs × 10 modalities; added 2026-08-20, see [mmreid-bench-kb.md](../field/mmreid-bench-kb.md). Affects the TL;DR's crowding argument and C13's standalone value; does not move the Pareto front, since it scores no rejection or calibration
