...
| Existing assets | VeRi-776 downloaded; SigLIP2-g and backbone embeddings cached; `reidbench` with mAP / CMC / per-query AP; 27-file ReID wiki | this wiki and the `reidbench` package |
...
|---|---|---|
| **C1** | Agglomerative VFMs as frozen ReID encoders | C-RADIOv4, RADIOv2.5, EUPE have never been measured on ReID; they are the one encoder family missing from 2601.20598. Protocol: [92](92-protocol-agglomerative-probe.md) — **zero-shot rows 2026-08-23** ([§12](92-protocol-agglomerative-probe.md#12-first-measurements--2026-08-23)), **linear / ArcFace / PCA probes 2026-08-24** ([§13](92-protocol-agglomerative-probe.md#13-the-probes--2026-08-24)): 84 rows, frozen C-RADIOv4 + one affine head reaches 0.7181 mAP / 0.8872 R1 on `market1501/official@1`. **The teacher ablation (§7) — the part that answers H1 and H2 — is still unrun**, and it is now the only thing left that is expensive in thought rather than in compute |
